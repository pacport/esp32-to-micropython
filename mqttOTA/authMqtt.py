...
def connect():
    ...
    try:
        ...
        print(thingName)
        mqtt_client = AWSMqtt(client_id, AWSConfig.endpoint, ssl_params, thingName, None)
        return mqtt_client
    except:
    ...
    with open(private_cert, 'r') as f:
        cert = f.read()
    ssl_params = {"key":key, "cert":cert, "server_side":False}
    mqtt_client = AWSMqtt(client_id, AWSConfig.endpoint, ssl_params,subscribe_cb = subscribe_callback)
