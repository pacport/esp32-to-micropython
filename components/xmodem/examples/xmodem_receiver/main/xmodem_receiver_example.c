...
static esp_err_t xmodem_ota_end(void)
{
    ...
    if (ota_end_err != ESP_OK) {
        ESP_LOGE(TAG, "esp_ota_set_boot_partition failed! err=0x%d", ota_end_err);
        return ota_end_err;
    }
...
static esp_err_t xmodem_receiver_event_handler(esp_xmodem_event_t *evt)
{
    switch(evt->event_id) {
        case ESP_XMODEM_EVENT_ERROR:
            ESP_LOGI(TAG, "ESP_XMODEM_EVENT_ERROR, err_code is 0x%x,%d", esp_xmodem_get_error_code(evt->handle), esp_get_free_heap_size());
            ota_init = false;
            xmodem_ota_end();
        ...
        case ESP_XMODEM_EVENT_ON_RECEIVE_DATA:
            ESP_LOGD(TAG, "ESP_XMODEM_EVENT_ON_RECEIVE_DATA, %d, %d", esp_get_free_heap_size(), evt->data_len);
            break;
