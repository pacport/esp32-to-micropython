...
//extern esp_err_t i2c_bus_lock(void);
//extern esp_err_t i2c_bus_unlock(void);
...
static sccb_lock_cbr  sccb_i2c_cbr = 0;

static void sccb_bus_lock(void)
{
    if (sccb_i2c_cbr) {
        (*sccb_i2c_cbr)(1);
    }
...
static void sccb_bus_unlock(void)
{
    if (sccb_i2c_cbr) {
        (*sccb_i2c_cbr)(0);
    }
...
int SCCB_Use_Port(int i2c_num, sccb_lock_cbr cbr) { // sccb use an already initialized I2C port
