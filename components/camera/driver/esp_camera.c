...
static esp_err_t camera_probe(const camera_config_t *config, camera_model_t *out_camera_model)
{
    ...
    *out_camera_model = CAMERA_NONE;
    ...
    if (config->pin_sccb_sda != -1) {
        ...
        ret = SCCB_Init(config->pin_sccb_sda, config->pin_sccb_scl, config->sccb_i2c_cbr);
    } else {
        ESP_LOGD(TAG, "Using existing I2C port");
        ret = SCCB_Use_Port(config->sccb_i2c_port, config->sccb_i2c_cbr);
    }

    if(ret != ESP_OK) {
        ESP_LOGW(TAG, "sccb init skipped");
